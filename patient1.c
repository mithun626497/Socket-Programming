...
* 1)  Upon boot patient1.c will read the input text file patient1.txt and store the username and password in     * 
*     memory.												         *
* 2)  It creates a TCP socket to the server running on static port 21198					 *
* 3)  Once TCP connection is created it sends message 'authenticate patient1 pasword'				 *
* 4)  If server replies back success for authentication then it proceed to phase2 and request for appointment.   *
* 5)  If server replies failure then patient1 will terminate the connection and exit.				 *
...
* 8)  If patient 1 receives 'notavailable' from server then it closes TCP connection and stop the process.       *
* 9)  Patient1 will close the TCP socket and will create a UDP socket for the doctor's port number.  		 *
* 10) Patient1 will send the insurance details in a message 'insurance#' and receives the estimated cost of      *
*     appointment.												 *
* 11) After receiving the estimated cost of appointment, patient1 will close UDP socket and will stop all the    *
*     running processes. 											 *
*     														 * 
*****************************************************************************************************************/
#include <stdio.h>
...
#define SRVPORT "21198" //The HealthCenterServer static port for which the patient will be connecting to.
...
int main(void)
{
	...
	char portname[20]="";
	char docName[10];
	/* Open patient1.txt in read mode to read the login credentials of patient1*/
   	fp1=fopen("patient1.txt","r");
  	if(NULL==fp1)
   	{
		printf("ERROR: fopen() returning NULL for patient1.txt\n");
		return 1;
   	}  
   
   	/*Read the username and password of patient1 and store it in usr and pwd respectively*/
	fscanf(fp1,"%s %s",usr,pwd);
	fclose(fp1);	

	/* Open patient1insurance.txt in read mode to read the insurance type */
	fp2 = fopen("patient1insurance.txt","r");
	if(NULL==fp2)
	{
		printf("ERROR: fopen() returning NULL for patient1insurance.txt\n");
		return 1;
	}	
	...
	{	
		...
		printf("Phase 1: Patient 1 has the TCP port number %d and ip address %s.\n",ntohs(sin.sin_port),ipstr);
	}
	...
	{
		printf("Phase 1: Authentication request from Patient 1 with username %s and password %s "
	...
	{	
		...
		else
		{
			...
			//if response is failure then display no appointments are available		
			...
			{	
				//if response is success then display the available appointments.
				...
				while(valid!=1)
				{	
					//ask the user to select one of the listed appointment indexes
				...
				//sending the selected index
				if (send(streamSockFd,&index,sizeof index, 0) == -1)
					perror("send error!!!");
		
			}	
		}
		//receive the response for the requested appointment index
		if ((bytesRecvd = recv(streamSockFd, buf, BUFFSIZE-1, 0)) == -1)
		{
			perror("recv error!!!");
			freeaddrinfo(servinfo); 
		...
		else
		{
			buf[bytesRecvd] = '\0';
			token=strtok(buf," ");
			//check if the appointment is available or not. If available display it.
			if(strcmp(token,"notavailable"))
			{
				...
				printf("Phase 2: The requested appointment is available and "
					"reserved to Patient1. The assigned doctor port is %s\n",dp);
			}
			else
			{
				//else print the requested appointment is not available
	...
	//create a UDP socket with the port number of doctor received from the phase 1
	...
	{	
		...
		{
			close(dgramSockFd);
			perror("patient1: UDP bind Failed");
			continue;
		}
	...
	if (getsockname(dgramSockFd, (struct sockaddr *)&sin, &len) == -1)
		...
		{	
			addr = &(sin.sin_addr);
			printf("Phase 3: Patient 1 has dynamic UDP port number %u and IP address %s\n",ntohs(sin.sin_port),ipstr);
		}

	

	//send the insurance type to the doctor port received in the previous phase 
	...
	{
		printf("Phase 3: The cost estimation request from patient1 with insurance plan %s has "
			"been sent to the doctor with \n port number %s and ip address %s\n",insurance,dp,ipstr);
	}
	//receive the response for the insurance request sent
	...
	buf[numbytes] = '\0';
	//if the response is not invalid then display the estimated cost
	if(strcmp(buf,"invalid"))
	{
		printf("Phase 3: Patient 1 receives %s$ estimation cost from doctor with port "
			"number %s and name %s\n",buf,dp,docName);
	}	
	else	
	{
		//if the response is invalid then display invalid insurance type.
